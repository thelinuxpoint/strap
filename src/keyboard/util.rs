...
pub trait HidDevice {
    ...
    fn set_report(&mut self, report_type: ReportType, report_id: u8, data: &[u8]) -> Result<(), ()>;
...
impl<B: UsbBus, D: HidDevice> HidClass<'_, B, D> {
    pub fn new(device: D, alloc: &UsbBusAllocator<B>) -> HidClass<'_, B, D> {
        HidClass {
            ...
            endpoint_interrupt_in: alloc.interrupt(8, 10), // this returns endpoint struct
...
/* The Trait ( UsbClass ) Implementation for
        @HidClass 
*/
